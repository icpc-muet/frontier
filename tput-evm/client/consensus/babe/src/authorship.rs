...
pub fn claim_slot(
	...
	let authorities = epoch
		...
		.collect::<Vec<_>>();
	claim_slot_using_keys(slot, epoch, keystore, &authorities, p_author)
}
...
pub fn claim_slot_using_keys(
	...
	claim_primary_slot(slot, epoch, epoch.config.c, keystore, keys, p_author).or_else(|| {
...
fn claim_primary_slot(
	...
	// w3v changes
	let author_id = authorities.get(p_author as usize).expect(
		"authorities not empty; index constrained to list length;this is a valid index; qed",
	);

	let expected_author = &author_id.0.clone();
...
mod tests {
	...
	fn claim_secondary_plain_slot_works() {
		...
		assert!(claim_slot(10.into(), &epoch, &keystore, 0u32).is_none());

		epoch.authorities.push((valid_public_key.into(), 10));
		assert_eq!(claim_slot(10.into(), &epoch, &keystore, 0u32).unwrap().1, valid_public_key.into());
	}
}
