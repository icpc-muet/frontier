	...
	Error: std::error::Error + Send + From<ConsensusError> + From<I::Error> + 'static,
{
	...
	async fn claim_slot(
		...
		// call proposed_author function
		// get the proposed slot author
		// W3v Changes, Author: Zubair Buriro
		// This line is added to call new consensus function proposed_author to get current slot author, 
		// as per custom W3v schedule
		// slot_author function is called with 03 parameters now, including p_author: u32
		let p_author = proposed_author(self.client.as_ref(), &BlockId::Hash(parent_header.hash()), slot).unwrap();
...
/// W3v Changes, Author: Zubair Buriro
/// New function defined to get proposed author index from runtime api
/// This function is called from the slot worker to get the proposed author index
/// This function returns either the proposed author index or 0
fn proposed_author<B, C>(client: &C, at: &BlockId<B>, slot: Slot) -> Result<u32, u32>
where
	// A: Codec + Debug,
	B: BlockT,
	C: ProvideRuntimeApi<B>,
