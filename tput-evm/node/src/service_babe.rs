...
pub fn create_extrinsic(
	...
	function: impl Into<frontier_template_runtime::RuntimeCall>,
	nonce: Option<u32>,
) -> frontier_template_runtime::UncheckedExtrinsic {
	...
	let period = frontier_template_runtime::BlockHashCount::get()
		.checked_next_power_of_two()
		.map(|c| c / 2)
		.unwrap_or(2) as u64;
	let tip = 0;
	let extra: frontier_template_runtime::SignedExtra = (
		frame_system::CheckNonZeroSender::<frontier_template_runtime::Runtime>::new(),
		frame_system::CheckSpecVersion::<frontier_template_runtime::Runtime>::new(),
		frame_system::CheckTxVersion::<frontier_template_runtime::Runtime>::new(),
		frame_system::CheckGenesis::<frontier_template_runtime::Runtime>::new(),
		frame_system::CheckEra::<frontier_template_runtime::Runtime>::from(generic::Era::mortal(
			period,
			best_block.saturated_into(),
		)),
		frame_system::CheckNonce::<frontier_template_runtime::Runtime>::from(nonce),
		frame_system::CheckWeight::<frontier_template_runtime::Runtime>::new(),
		pallet_asset_tx_payment::ChargeAssetTxPayment::<frontier_template_runtime::Runtime>::from(
			tip, None,
		),
	);

	let raw_payload = frontier_template_runtime::SignedPayload::from_raw(
		function.clone(),
		extra.clone(),
		(
			(),
			frontier_template_runtime::VERSION.spec_version,
			frontier_template_runtime::VERSION.transaction_version,
			genesis_hash,
			best_hash,
			(),
			(),
	...
	frontier_template_runtime::UncheckedExtrinsic::new_signed(
		function,
		sp_runtime::AccountId32::from(sender.public()).into(),
		frontier_template_runtime::Signature::Sr25519(signature),
		extra,
	)
}
*/
