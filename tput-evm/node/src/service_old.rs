...
use sc_client_api::{BlockBackend, StateBackendFor, AuxStore, UsageProvider};
use sc_consensus::BasicQueue;
use sc_executor::{ NativeElseWasmExecutor, NativeExecutionDispatch };
use sc_service::{error::Error as ServiceError, Configuration, PartialComponents, TaskManager};
use sc_telemetry::{Telemetry, TelemetryHandle, TelemetryWorker};
use sp_api::{ConstructRuntimeApi, TransactionFor, BlockId, ProvideRuntimeApi };
use sp_consensus_babe::{AuthorityId, BabeConfiguration, Epoch};
use sc_consensus_babe::{self, SlotProportion, BabeApi, aux_schema, BabeLink};
...
use frontier_template_runtime::{opaque::Block, opaque::Header, Hash, TransactionConverter, RuntimeApi};
...
//pub type BFullClient =
//	sc_service::TFullClient<Block, RuntimeApi, NativeElseWasmExecutor<dispatch>>;
...
// type BabeLink<Client> = sc_consensus_babe::BabeLink<Block>;
// type BabeBlockImport<Client> = sc_consensus_babe::BabeBlockImport<Block, FullClient<RuntimeApi, Executor>, GrandpaBlockImport<Client>>;

// type define for BabeApi
//type BabeRuntimeApi<B> = sp_api::ConstructRuntimeApi<Block, B, B::RuntimeApi, sp_consensus_babe::BabeApi<Block, B>>;

/*
pub fn get_babe_config<RuntimeApi, Executor>(client: &FullClient<RuntimeApi, Executor>) -> BabeConfiguration 
where 
	...
	FullClient<RuntimeApi, Executor>: AuxStore + ProvideRuntimeApi<Block> + UsageProvider<Block>,
	RuntimeApi: BabeApi<Block>,
{
	let best_hash = client.chain_info().best_hash;
	// call runtime api to get the babe configuration
	let babe_config = client
		.runtime_api()
		//.BabeApi()
		.configuration(&best_hash)
		.map_err(|e| ServiceError::RuntimeApiError(e.into()))?;
...
pub fn new_partial<RuntimeApi, Executor, BIQ>(
	...
	PartialComponents<
		...
		(
			...
			//	BabeLink<FullClient<RuntimeApi, Executor>>,
			Arc<FrontierBackend>,
			//(
			//	sc_consensus_babe::BabeBlockImport<Block, FullClient<RuntimeApi, Executor>, GrandpaBlockImport<Client>>,
...
where
	...
	RuntimeApi::RuntimeApi:
		BaseRuntimeApiCollection<StateBackend = StateBackendFor<FullBackend, Block>>,
	Executor: NativeExecutionDispatch + 'static,
	//sc_consensus_babe::BabeLink<Block>: 'static,
	//FullClient<RuntimeApi, Executor>: AuxStore + ProvideRuntimeApi<Block> + UsageProvider<Block>,
    //RuntimeApi: BabeApi<Block>,
	/*
	BIQ: FnOnce(
		...
		Arc<FrontierBackend>,
		//BabeLink<FullClient<RuntimeApi, Executor>>,
		//BabeBlockImport<FullClient<RuntimeApi, Executor>>,
		// implement for BabeApi
		//dyn BabeRuntimeApi<FullBackend>,
	)
	 -> Result<
		(
			BasicImportQueue<FullClient<RuntimeApi, Executor>>,
	...
	*/
{
	...
	let block_id = client.block_id().expect("Failed to get the block ID");

	// Get the runtime API.
	let runtime_api = RuntimeApi::construct_runtime_api(&*client)?;
	let babe_config = BabeApi::<Block<Header<u32, BlakeTwo256>, OpaqueExtrinsic>>::configuration(&runtime_api, &block_id)
    .expect("Failed to fetch BABE genesis configuration");

	/*
	let babe_config = sc_consensus_babe::configuration(&*client).expect("config available");
	let dummy_epoch_changes = SharedEpochChanges::<Block, Epoch>::new(); // Create a dummy epoch_changes.

	
	let babe_link = BabeLink::<Block> {
		epoch_changes: dummy_epoch_changes,
		config: babe_config,
	};*/
	*/
	...
	//let babe_config = get_babe_config::<RuntimeApi, Executor>(&*client);
	...
	// get epoch changes from client and client configuration


	let epoch_changes = aux_schema::load_epoch_changes::<Block, _>(&*client, &babe_config)?;
	let link = BabeLink { epoch_changes: epoch_changes.clone(), config: config.clone() };

    // Access the epoch changes and config
    //let epoch_changes = babe_link.epoch_changes();
    let babe_config = link.config();
	
	

	let (block_import, _babe_link) = sc_consensus_babe::block_import(
		babe_config, //sc_consensus_babe::configuration(&*client)?,
		grandpa_block_import,
		client.clone(),
	)?;

	let slot_duration = link.config().slot_duration();
	...
	/*
	let (import_queue, block_import) = build_babe_grandpa_import_queue(
		client.clone(),
		config,
		eth_config,
		&task_manager,
	...
	*/
	...
	Ok(PartialComponents {
		...
		import_queue,
		transaction_pool,
		other: (telemetry, Box::new(block_import), grandpa_link, frontier_backend),
	})
}
...
pub fn build_babe_grandpa_import_queue<RuntimeApi, Executor>(
	...
	//babe_link: BabeLink<FullClient<RuntimeApi, Executor>>,
	//_babe_block_import: GrandpaBlockImport<FullClient<RuntimeApi, Executor>>,
) -> Result<
	(
		BasicImportQueue<FullClient<RuntimeApi, Executor>>,
		BoxBlockImport<FullClient<RuntimeApi, Executor>>,
	...
	Executor: NativeExecutionDispatch + 'static,
{
	...
	let babe_config = sc_consensus_babe::configuration(&*client).expect("config available");
	let (_, babe_link) = sc_consensus_babe::block_import(babe_config.clone(), grandpa_block_import, client.clone())
		.expect("can initialize block-import");
	
	//let slot_duration = sc_consensus_babe::slot_duration(&*client)?;
	...
	let import_queue = sc_consensus_babe::import_queue(
		sc_consensus_babe::ImportQueueParams {
			...
			telemetry,
			compatibility_mode: sc_consensus_babe::CompatibilityMode::None,
		},
	)
	...
	*/
	...
	if role.is_authority() {
		...
		*/
		...
		// let slot_duration = sc_consensus_babe::configuration::slot_duration(&*client)?;
		// let slot_duration = babe_link.config().slot_duration();

		// get slot_duration from client.runtimeApi
		//let slot_duration = client.runtime_api().slot_duration().map_err(|e| {
		//	sc_cli::Error::Service(format!("Failed to get slot_duration from runtimeApi: {:?}", e))
		//})?;
		
		
		//let babe_config = sc_consensus_babe::configuration(&*client).expect("config available");
		//let (_, babe_link) = sc_consensus_babe::block_import(babe_config.clone(), block_import.clone(), client.clone())
		//.expect("can initialize block-import");
		...
		/*
		let aura = sc_consensus_babe::start_babe::<Pair, _, _, _, _, _, _, _, _, _, _>(
